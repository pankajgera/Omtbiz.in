<template>
  <div class="row">
    <div class="col-md-4 reports-tab-container">
      <div class="row">
        <div class="col-md-8">
          <label class="report-label">{{ $t('reports.banks.date_range') }}</label>
          <base-select
...
export default {
  ...
  mounted () {
    this.siteURL = `/reports/banks/${this.getSelectedCompany.unique_hash}`
  ...
  methods: {
    ...
    async getReports (isDownload = false) {
      ...
      this.url = `${this.siteURL}?
        from_date=${moment(this.formData.from_date).format('DD/MM/YYYY')}&to_date=${moment(this.formData.to_date).format('DD/MM/YYYY')}`
      return true
    },
    downloadReport () {
      if (!this.getReports()) {

<template>
<div class="vue-editable-grid">
    <div class="grid-tools">
        <div class="grid-tools-left">
            <slot name="header"></slot>
            <paginate class="grid-tool" :page="page" :pages="pages" :page-count="pageCount" :total-rows="rowDataFiltered.length" @count-changed="pageCount=$event" @prev="page--" @next="page++" v-if="pageCount"></paginate>
            <filters class="grid-tool" :filters="filter" :columnDefs="columnDefs" @remove="removeFilter"></filters>
        </div>
        <div class="grid-tools-right">
            <slot name="header-r"></slot>
        </div>
    </div>
    <div class="grid-table-container" ref="container">
        <table class="grid-table" ref="table" :class="{ filters: enableFilters }">
            <thead ref="head">
                <tr class="headers-row" :style='{ "grid-template-columns": gridTemplateColumns }'>
                    <th v-for="(column, index) in columnDefs" :key="index" :class="{ sortable: column.sortable, sorting: column.field === sortByColumn, descending: sortByDesc }" @click="sort(column)">
                        <span class="header-content">{{ column.headerName }}</span><span class="resize-handle" @mousedown="initResize(column, $event)" @click.stop></span>
                    </th>
                </tr>
                <tr class="filters-row" :style='{ "grid-template-columns": gridTemplateColumns }' v-if="enableFilters">
                    <th v-for="(column, index) in columnDefs" :key="index" :class="{ filter: column.filter }">
